...
- USB lighting controls on the Basilisk V3 Pro and Basilisk V3 35K now hydrate from OpenSnek's remembered last-used color/effect state so the editor matches the mouse more closely on connect, without restoring that lighting state back onto the hardware.
- Choosing `Default` for special USB button slots now applies the real default action semantics on write, so the 35K top DPI button once again behaves like `DPI Cycle` without forcing you to assign `DPI Cycle` explicitly.
