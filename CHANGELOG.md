...
### Fixed
- `./run.sh` now asks the app to quit cleanly before falling back to process killing, and successful button-binding writes now flush their `UserDefaults` save immediately. That makes local restart loops more reliable for recently changed remaps, especially when relaunching right after a bind change.
